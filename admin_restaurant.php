...
$x->database('foodplaces','itemid','name');
$x->permissions('add',1);
$x->permissions('mod',1);
$x->permissions('del',1);
...
$x->format('table');
	$x->format('row');
		$x->format('cell');
			$x->add_text('city',0,1,0,get_lang('desc_city'),array(),100);
		$x->format('endcell');
		$x->format('cell');
			$x->add_text('state',0,1,0,get_lang('desc_state'),array(),2);
		$x->format('endcell');
		$x->format('cell');
			$x->add_text('zipcode',0,1,0,get_lang('desc_zipcode'),array(),12);
		$x->format('endcell');
	$x->format('endrow');
$x->format('endtable');
$x->add_text('phone',0,1,0,get_lang('desc_phone'),array(),20);
$x->add_text('traveltime',0,1,0,get_lang('desc_traveltime'),array(),100);
$x->add_checkbox('delivery',0,1,0,get_lang('desc_delivery'),array(),0);
