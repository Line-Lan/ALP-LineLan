...
if ($x->is_secure()&&!$ip_registered) {
	...
	if (empty($_POST)) { ?>
		...
		<input type="text" name="username" size=40 maxlength=40 style="width:99%"><br /><br />
		<?php
        ...
        <input type="text" name="first_name" maxlength="30" style="width:99%">
          <br />
          <font size="1">
            <strong>last name</strong>
            <br />
          </font>
          <input type="text" name="last_name" maxlength="30" style="width:99%">
            <br /> <br />
			<?php
			}
				if( (ALP_TOURNAMENT_MODE && $master['internetmode']) || !ALP_TOURNAMENT_MODE) { ?>
				<font size=1><strong>e-mail</strong><?php 
				if(!ALP_TOURNAMENT_MODE) { 
				}
				?><br /></font>
				<input type="text" name="email" maxlength=60 style="width:99%"><br /><br />
				<?php
			} ?>
<!--
            <br />
			<font size=1><strong>how many of the other gamers at the event do you think you can kill in a one-on-one in your favorite game?</strong> (for random team tournaments -- be honest)<br /></font>
...
-->
		...
		if(!ALP_TOURNAMENT_MODE) { ?>
			...
			<font size=1><strong>allow others to see your ip address?</strong> (for file sharing or ftp server links)<br /></font>
		...
		<font size=1>for maximum security, enable javascript. do not be alarmed, your password will autoencrypt when you click the register button.</font><br /><br />
