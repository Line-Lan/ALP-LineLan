...
if ($x->is_secure() && $toggle['uploading']) { 
	...
	if (empty($_POST)) {
		...
		please only upload files documenting or pertaining to the event.  keep the total size of all files combined less than <?php echo round($master['max_file_upload_size']/1048576,1); ?> MB.  you will <b>not</b> have permission to delete or modify files that have been uploaded.  uploaded images must be in the GIF, the JPEG or the PNG format.<br />
		<br />
			...
			for ($i=0;$i < $valid->get_value('num_of_files'); $i++) {
				if (!empty($_FILES['userfile']['name'][$i]) && $_FILES['userfile']['error'][$i] != 4) {
					...
					if ($valid->get_value('file_type_'.$i) == 'pictures' || $valid->get_value('file_type_'.$i) == 'screenshots' || $valid->get_value('file_type_'.$i) == 'benchmarks') {
						...
						if ($_FILES['userfile']['size'][$i]>$master['max_file_upload_size'] || $_FILES['userfile']['error'][$i] == 1 || $_FILES['userfile']['error'][$i] == 2) {
							$valid->add_error('that file is too big to upload.');
						}
					}
					else
