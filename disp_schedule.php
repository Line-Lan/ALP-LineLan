...
if ($x->is_secure() && $toggle['schedule']) { 
	...
	if(!ALP_TOURNAMENT_MODE && $toggle["foodrun"]) $x->add_related_link('modify all foodruns','admin_foodrun.php',2);
	...
	if(!ALP_TOURNAMENT_MODE) { ?>
		the schedule on the sidebar lists tournaments and miscellaneous schedule items occuring in the next hour.  the schedule appearing below lists everything.<br />
		<br />
		<?php
	} ?>
