...
if ($x->is_secure()) { 
	if (current_security_level() >= 1) { 
		...
		if (empty($_POST)) { 
			...
			<?php
			if (!$master['authbyiponly']) { ?>
				<font size="1"><a href="register.php"><?php echo get_lang('cp_register') ?></a></font><br />
			...
			<div align="right"><input type="submit" value="<?php get_lang('log in',1) ?>"  class="formcolors"<?php if(!$master['authbyiponly']) { ?> onClick="doLogin(); return true;"<?php } ?> /></div>
			</form>
			</td></tr></table>
			<?php
			if(!$master['authbyiponly']) { ?>
