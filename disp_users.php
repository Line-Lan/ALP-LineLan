...
if($x->is_secure()) { 
	...
	if($row = $dbc->database_fetch_assoc($data)) {
		...
		<?php if(!empty($row["gender"])) { ?><tr><td><a href="users.php?show=1" style="color: <?php echo $colors["blended_text"]; ?>"><font class="sm">gender</font></a> </td><td><?php echo $row["gender"]; ?><br /></td></tr><?php } ?>
		<tr><td><a href="users.php?show=6" style="color: <?php echo $colors["blended_text"]; ?>"><font class="sm">time of arrival</font></a></td><td><?php echo disp_datetime(strtotime($row["date_of_arrival"]), 0); ?></td></tr>
		<?php if($row["display_ip"]||current_security_level()>=2) { ?><tr><td><a href="users.php?show=7" style="color: <?php echo $colors["blended_text"]; ?>"><font class="sm">ip address</font></a></td><td><?php echo $row["recent_ip"]; ?><br /></td></tr><?php } ?>
		...
		if($toggle['gamingrigs']) { ?>
			...
			?></td></tr>
			<tr><td><font color="<?php echo $colors["blended_text"]; ?>" size=1>processor</font> </td><td><?php echo $row["comp_proc"].' '.$row['comp_proc_type'].(!empty($row['comp_proc_spd'])?' @ '.$row['comp_proc_spd'].' MHz':''); ?><br /></td></tr>
			<tr><td><font color="<?php echo $colors["blended_text"]; ?>" size=1>memory</font> </td><td><?php echo (!empty($row['comp_mem'])?$row["comp_mem"].' GB ':'').$row['comp_mem_type']; ?><br /></td></tr>
