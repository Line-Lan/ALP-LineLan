...
class display {
	...
	function display_selector(& $URL_handler, $toggle_show = 1, $toggle_per = 1) { ?>
		...
		<?php
		if($toggle_show) { 
			?>show: <select name="show" style="width: 200px; font: 10px consolas;"><?php
		...
		if($toggle_per) {
			?>per page: <select name="per" style="font: 10px consolas;"><?php
		...
		?></font>
		<input type="submit" value="go" style="font: 10px consolas;" class="formcolors" />
		</form>
		<br />
		<?php
	}
