...
class sof1 extends gsQuery
{
  ...
  { 
	...
	break;
      case "mapname":
        $this->gametype=strtoupper(substr($rawdata[$i],0,2));
	$this->mapname=substr($rawdata[$i],3,(strlen($rawdata[$i])-3));
	break;
      case "capturelimit":	
      case "scorelimit":  
...
$retval="<table cellspacing=0 cellpadding=0 width=\"100%\">"
  ...
  . "		<tr><td style=\"padding-right: 5px;\">".checkmark()." <font class=\"color\">Control Bonus:</font></td><td>".$gameserver->rules["control_bonus"]."</td></tr>"
  . "		<tr><td style=\"padding-right: 5px;\">".checkmark()." <font class=\"color\">Control Limit:</font></td><td>".$gameserver ->rules["control_limit"]."</td></tr>"
