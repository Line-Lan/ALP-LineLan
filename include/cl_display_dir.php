		...
		if ($handle = @opendir(getcwd().'/files/'.$types[$dir][0].'/')) { 
			...
			while (false!== ($file = @readdir($handle))) {
				...
				if ($file != '.' && $file != '..' && $file != 'CVS') { ?>
					<tr class="sm"><td width="90%"><?php if($label) { ?>&nbsp;&nbsp;&nbsp; <?php } ?><a href="files/<?php echo $types[$dir][0]; ?>/<?php echo $file; ?>"<?php echo (!$label?" target=\"_top\"":""); ?>><?php echo (!$label&&strlen($file)>23?substr($file,0,23)."...":$file); ?></a> </td><td>&nbsp;&nbsp;<font color="<?php echo $colors['blended_text']; ?>"><?php echo $filesize.$holder; ?></font><br /></td></tr>
					<?php
				}
			}
