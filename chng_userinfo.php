...
$x->add_text('username',0,1,0,'username',array(),30);
$x->add_text('first_name',0,1,0,'first name',array(),30);
$x->add_text('last_name',0,1,0,'last name',array(),30);
$x->add_select('proficiency',0,!$master['proficiencylock'],0,'gamer proficiency rating (how many of the other people at the event do you think you can kill in a one-on-one in your favorite game?) (used for random team tournaments)',array(),$array);
...
if($toggle["gamingrigs"]) {
	...
	$x->format("table");
		$x->format("row");
			...
			$x->format("endcell");
			$x->format("cell");
				$x->add_text("comp_proc_spd",0,1,0,"cpu MHz</b> (numeric only)",array(),60);
			$x->format("endcell");
		$x->format("endrow");
	$x->format("endtable");
	...
	$x->format("table");
		...
		$x->format("endrow");
	$x->format("endtable");
	$x->add_select("comp_hdstorage",0,1,0,"hard disk capacity",array(),$x_storage);
	$x->format("table");
		$x->format("row");
			$x->format("cell");
			...
			$x->format("endcell");
			$x->format("cell");
	$x->add_text("comp_gfx_type",0,1,0,"model number",array(),60);
			$x->format("endcell");
		$x->format("endrow");
	$x->format("endtable");
